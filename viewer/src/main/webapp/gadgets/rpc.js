...
if (!window['gadgets']['rpc']) { // make lib resilient to double-inclusion
  ...
  gadgets.rpc = function() {
    ...
    var isChild =
    	// [AD] Removed "parent" URL parameter restriction 
//    	!!gadgets.util.getUrlParameters().parent &&
    ...
    function callSameDomain(target, rpc) {
      ...
      if (!sameDomain[target] || (sameDomain[target] !== _flagCrossOrigin &&
        ...
        // [AD] make sure targetRelay is available before comparison
        if (!!targetRelay && getOrigin(targetRelay) !== getOrigin(window.location.href)) {
    ...
    function setupContainedContext(rpctoken, opt_parent) {
      ...
      // [AD] All the above code doesn't work; setting relayUrl manually
      if (params['parent']) { 
    	  setRelayUrl('..', params['parent']);
      }
    }
